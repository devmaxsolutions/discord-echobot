...
class EchoBot {
	constructor() {
		...
		if (!this.loadConfiguration()) return;
	 ...
	 */
	private async onDiscordClientMessageReceived(
		...
		for (let redirect of matchingRedirects) {
			...
			if (redirect.options && redirect.options.minLength) {
				if (
					!body.embed &&
					(!body.contents || body.contents.length < redirect.options.minLength)
				) {
					logger.info(
						`Dropping message from ${
							message.author.username
			...
			if (!body.contents && !body.embed) {
				logger.info(
					`Dropping message from ${
						...
						message.channel
					)} as their message would be empty due to redirect options.`
				);
				continue;
			}
			...
			for (let destination of redirect.destinations) {
				...
				if (destChannel == null) {
					Promise.reject(
						`Could not redirect from channel ID ${message.channel.id} to channel ID ${destination}: Destination channel was not found.`
					);
					return;
				} else if (!(destChannel instanceof TextChannel)) {
					Promise.reject(
						`Could not redirect from channel ID ${message.channel.id} to channel ID ${destination}: Destination channel is not a text channel.`
					);
					return;
				}
				...
				logger.info(
					`Redirecting message by ${
						message.author.username
					} from ${this.explainPath(message.channel)} to ${this.explainPath(
						destChannel
					)}`
				);
				...
				let options: Discord.MessageOptions = {
					nonce: this.generateNonce(),
					files: redirect.options.copyAttachments
						? message.attachments.map((attachment) => {
								return new Discord.Attachment(
									attachment.url,
									attachment.filename
								);
						  })
	...
	private createHeader(
		...
		if (redirect.options && redirect.options.richEmbed) {
			...
			if (redirect.options.includeSource) {
				richEmbed.addField("Info", `${message.guild.name} | ${message.url}`);
			}
			return richEmbed;
		} else {
			...
			if (redirect.options && redirect.options.includeSource) {
				destinationMessage += `**${message.guild.name}** | ${message.url}`;
			}
	...
	private createBody(
		message: Discord.Message,
		redirect: EchobotRedirect
	): { contents?: string; embed?: Discord.RichEmbed } {
	...
	private generateNonce(): string {
		let nonce = Math.round(
			Math.random() * Math.floor(Number.MAX_SAFE_INTEGER / 10)
		).toString(); // About 15 digits
